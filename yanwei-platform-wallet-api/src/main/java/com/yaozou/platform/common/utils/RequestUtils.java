...
public class RequestUtils {
	 ...
	 * cookie中的BTSSIONID名称
	 */
	public static final String BTSSIONID_COOKIE = "BTSSIONID";
	...
	public static String getRequestedSessionId(HttpServletRequest request) {
		...
		if (request.isRequestedSessionIdFromURL() || StringUtils.isBlank(ctx)) {
			...
			// 手动从cookie获取
			Cookie cookie = CookieUtils.getCookie(request, BTSSIONID_COOKIE);
			if (cookie != null) {
	...
	public static void main(String[] args) {
		...
		// 商品惠币数计算
		/*
		 * Double rate=processScoreRate();
		 * System.out.println("商品惠币数分配率计算:"+rate); Integer[]
		 * productsScore={25,50,425};//商品可以使用惠币数 500
		 * 
		 * for(Integer productScore:productsScore) {
		 * 
		 * System.out.println("商品惠币数分配数:"+rate*productScore); }
		 */

	/*	Integer orderRatioScore = 10;
		Integer priceOrder = 10000;
		Integer scoreToRmb = 1000;

		long priceScore = priceOrder * orderRatioScore * scoreToRmb / 100 / 100;

		System.out.println(orderRatioScore / 100);

		System.out.println(priceScore);*/
		
		System.out.println(getProductPriceScore(50.0,4));
	}

	/**
	 * 商品分配惠币惠币计算 人民币+惠币=999人民币+500惠币
	 * 
	 * 返回计算公式
	 * 
	 * 材料运杂费分配率：＝5568/(12600+4800）搜索＝0.32 甲材料分配的运杂费=12600*0.32=4032
	 * 乙材料分配的运杂费=4800*0.32=1536
	 * 
	 * 
	 */
	public static Double processScoreRate() {
		// 商品惠币
		Integer scoreTotalNum = 600;
		// 商品数量
		Integer[] productsNum = { 1, 1, 1 };
		Integer[] productsScore = { 25, 50, 425 };// 商品可以使用惠币数 500
		// 计算出分配的惠币总数
		Integer productsTotalScore = 0;

		for (int i = 0; i < productsScore.length; i++) {
			// 商品可用积分数
			Integer productScore = productsScore[i];
			// 商品数量
			Integer productNum = productsNum[i];
			//
			productsTotalScore += productScore * productNum;
		}
		BigDecimal b1 = new BigDecimal(productsTotalScore);
		BigDecimal b2 = new BigDecimal(scoreTotalNum);
		// 计算出分配的惠币数占比量
		return b2.divide(b1, 4, BigDecimal.ROUND_HALF_EVEN).doubleValue();
	}
