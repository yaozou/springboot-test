 ...
 * 定时任务管理
 * @author luojianhong
 * @version $Id: TaskScheduleJobMapper.java, v 0.1 2017年10月9日 下午4:13:48 luojianhong Exp $
 */
@Mapper
public interface TaskScheduleJobMapper {

	@Select("select `id`, `cron_expression`, `method_name`, `is_concurrent`, `description`, `update_by`, `bean_class`, `create_date`, `job_status`, `job_group`, `update_date`, `create_by`, `spring_bean`, `job_name` from acc_task_schedule_job where id = #{id}")
	TaskScheduleJobDO get(Long id);
	
	@Select("<script>" +
	"select * from acc_task_schedule_job " + 
			"<where>" + 
	...
	@Select("<script>" +
	"select count(*) from acc_task_schedule_job " + 
			"<where>" + 
		  		  "<if test=\"id != null and id != ''\">"+ "and id = #{id} " + "</if>" + 
		  		  "<if test=\"cronExpression != null and cronExpression != ''\">"+ "and cron_expression = #{cronExpression} " + "</if>" + 
		  		  "<if test=\"methodName != null and methodName != ''\">"+ "and method_name = #{methodName} " + "</if>" + 
	...
	@Insert("insert into acc_task_schedule_job (`id`, `cron_expression`, `method_name`, `is_concurrent`, `description`, `update_by`, `bean_class`, `create_date`, `job_status`, `job_group`, `update_date`, `create_by`, `spring_bean`, `job_name`)"
	...
	@Delete("delete from acc_task_schedule_job where id =#{id}")
	int remove(Long id);
	
	@Delete("<script>"+ 
			"delete from acc_task_schedule_job where id in " + 
